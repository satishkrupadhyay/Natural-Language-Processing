...
#To remove stop word e.g.- stop word is a commonly used word (such as "the")
lords <- tm_map(lords, removeWords,stopwords("english"))
#For stem words e.g- friendships contains the stem friend
...
wordcloud(lords, scale = c(2,0.5), max.words = 100, random.order = FALSE, rot.per = 0.55, use.r.layout = FALSE, colors = brewer.pal(8,"Dark2"))
